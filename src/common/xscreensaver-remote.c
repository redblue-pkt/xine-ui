...
static Atom XA_SCREENSAVER_ID, XA_SCREENSAVER_STATUS, XA_SELECT;
static Atom XA_DEMO, XA_EXIT;
static Atom XA_BLANK, XA_LOCK;
      ...
      if (status < 0)
	{
	  char buf[1024];
          if (error_ret)
            ...
            {
              sprintf (buf, "%s: error waiting for reply", PACKAGE);
              perror (buf);
            }
	  return status;
	}
 ...
 */
/*
 * Initialize xscreensaver client.
 */
void xscreensaver_remote_init(Display *dpy) {
...
/*
 * Return 1 if xscreensaver already run.
 */
int is_xscreensaver_running(Display *dpy) {
  char *v = 0;
  Window window = find_screensaver_window (dpy, &v);
...
/*
 * Kill the running xscreensaver process .
 */
int xscreensaver_kill_server(Display *dpy) {

  return (xscreensaver_command (dpy, XA_EXIT, 0L, True, NULL));
}
