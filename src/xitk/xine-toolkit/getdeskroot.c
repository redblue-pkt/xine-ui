...
static Window __GetKDEDesktop(Display *display, int screen, 
  ...
  if (go_deeper && (XQueryTree(display, window, 
	...
	if (XGetWindowAttributes(display, children[i], &attributes)) {
	  if ((attributes.width >= (DisplayWidth(display, screen)/2)) && attributes.height > 0) {
	    /* Found it! */
...
static Window __GetNautilusDesktop(Display *display, int screen, Window window, int depth) {
  Window       rootReturn, parentReturn, *children;
  ...
  *clientparent = root;
  ...
  if ((background == None)
	...
	XFree((char *) newroot);
      ...
      else if ((background = __GetKDEDesktop(display, screen, children[i],
					     _NET_WM_WINDOW_TYPE,
