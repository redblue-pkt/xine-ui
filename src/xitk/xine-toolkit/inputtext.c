...
static void notify_keyevent_inputtext(xitk_widget_list_t *wl, xitk_widget_t *w, XEvent *xev) {
  ...
  if(w && ((w->widget_type & WIDGET_TYPE_MASK) == WIDGET_TYPE_INPUTTEXT)) {
	...
	if(modifier & MODIFIER_CTRL) {
	  ...
	  if(XGetSelectionOwner(private_data->imlibdata->x.disp, XA_PRIMARY) != None) {
	    printf("OWNER FOR SELECTION\n");
	    //       selection_paste(Xroot, XA_CUT_BUFFER0, False);
	    //////////////////////////////
	    
	    for (nread = 0, bytes_after = 1; bytes_after > 0; nread += ct.nitems) {
	      if ((XGetWindowProperty(private_data->imlibdata->x.disp,
				      private_data->imlibdata->x.root, XA_CUT_BUFFER0,
				      (nread / 4), 4096,
				      0, AnyPropertyType, &ct.encoding,
...
#ifdef MULTICHAR_SET
	      if (XmbTextPropertyToTextList(private_data->imlibdata->x.disp, &ct, &cl,
	  ...
	  _failed:
	    ...
	    prop = XInternAtom(private_data->imlibdata->x.disp, "VT_SELECTION", False);
#ifdef MULTICHAR_SET
	    ct = XInternAtom(Xdisplay, "COMPOUND_TEXT", False);
	    XConvertSelection(private_data->imlibdata->x.disp, XA_PRIMARY, ct, prop, TermWin.vt, tm);
#else
	    XConvertSelection(private_data->imlibdata->x.disp, XA_PRIMARY, 
			      XA_STRING, XA_CUT_BUFFER0, private_data->imlibdata->x.root,
			      CurrentTime);
#endif
	  }
