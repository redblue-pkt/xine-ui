...
static kbinding_entry_t default_binding_table[] = {
  ...
  { "Set position to -60 seconds in current stream.",
    "SeekRelative-60",        ACTID_SEEK_REL_m60            , "Left",     KEYMOD_CONTROL }, 
  { "Set position to +60 seconds in current stream.",
    "SeekRelative+60",        ACTID_SEEK_REL_p60            , "Right",    KEYMOD_NOMOD   },
  { "Set position to -15 seconds in current stream.",
    "SeekRelative-15",        ACTID_SEEK_REL_m15            , "Left",     KEYMOD_NOMOD   },
  { "Set position to +15 seconds in current stream.",
  ...
  { "Xineshot (External fullscreen dump - XShm only).",
    "Xineshot",               ACTID_XINESHOT                , "x",        KEYMOD_NOMOD   },
  ...
  { "Previous angle event.",
    "EventAnglePrior",        ACTID_EVENT_ANGLE_PRIOR       , "KP_7",     KEYMOD_NOMOD   },
  { "Next angle event.",
    "EventAngleNext",         ACTID_EVENT_ANGLE_NEXT        , "KP_1",     KEYMOD_NOMOD   },
  { "Select event.",
...
static void _kbindings_add_entry(kbinding_t *kbt, user_kbinding_t *ukb) {
  ...
  if(k) {
     ...
     */
    kbt->entry[kbt->num_entries] = (kbinding_entry_t *) xmalloc(sizeof(kbinding_t));
...
static void _kbindings_parse_section(kbinding_t *kbt, kbinding_file_t *kbdf) {
  ...
  if((kbdf->ln != NULL) && (*kbdf->ln != '\0')) {
    ...
    ukb = (user_kbinding_t *) xmalloc(sizeof(user_kbinding_t));
...
static void _kbinding_load_config(kbinding_t *kbt, char *file) {
  ...
  kbdf = (kbinding_file_t *) xmalloc(sizeof(kbinding_file_t));
...
static void _kbindings_display_kbindings(kbinding_t *kbt, int mode) {
  ...
  if(kbt == NULL) {
    printf("OOCH: key binding table is NULL.\n");
    return;
  }
  ...
  case LIRC_DISPLAY_MODE:
    fprintf(stdout, "##\n# Xine key bindings.\n");
    fprintf(stdout, "# Automatically generated by %s version %s.\n##\n\n", PACKAGE, VERSION);
...
static kbinding_t *_kbindings_init_to_default(void) {
  ...
  kbt = (kbinding_t *) xmalloc(sizeof(kbinding_t));

  for(i = 0; default_binding_table[i].action != NULL; i++) {
    kbt->entry[i] = (kbinding_entry_t *) xmalloc(sizeof(kbinding_entry_t));
  ...
  kbt->entry[i] = (kbinding_entry_t *) xmalloc(sizeof(kbinding_t));
